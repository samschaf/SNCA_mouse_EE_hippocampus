...
```{r volcano 1, eval=F}
results_G <- read.csv("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/hydroxy/WTEE_TGEE/linear_model/auto/CpG/lm_WTEE_TGEE.csv")

#CpGs with increased hmC in TG mice
hmC_up <- results_G[results_G$DB>0 & results_G$threshold==TRUE,"site"] #100 CpGs

#CpGs with decreased hmC in TG mice
hmC_down <- results_G[results_G$DB<0 & results_G$threshold==TRUE,"site"] #1314 CpGs
...
#DNAm
nrow(betas_mC <- all_EE[rownames(all_EE) %in% hmC_up,]) #100
...
nrow(betas_hmC <- betas_variable[rownames(betas_variable) %in% hmC_up,]) #100
...
####### CpGs with decreased hmC in TG mice 
#DNAm
nrow(betas_mC <- all_EE[rownames(all_EE) %in% hmC_down,]) #1314

#DNAhm
nrow(betas_hmC <- betas_variable[rownames(betas_variable) %in% hmC_down,]) #1314
all(rownames(betas_mC)==rownames(betas_hmC)) #TRUE
...
results_mC <- read.csv("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/auto/CpG/lm_WTEE_TGEE.csv")
nrow(mC_change <- results_mC[results_mC$Pr...t..<=0.01 & abs(results_mC$DB)>=0.1,]) #4128
length(mC_hmC <- mC_change$site[mC_change$site %in% hmC_down|mC_change$site %in% hmC_up]) #100 (all CpGs with increased hmC have a trend toward mC change, meeting effect size cutoff as well as nominal p-value cutoff)
