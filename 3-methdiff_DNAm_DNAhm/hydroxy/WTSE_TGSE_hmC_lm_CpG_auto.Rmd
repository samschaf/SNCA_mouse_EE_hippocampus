...
```{r volcano 1, eval=F}
results_G <- read.csv("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/hydroxy/WTSE_TGSE/linear_model/CpG/lm_WTSE_TGSE.csv")
...
#Volcano plot
results_G_complete$threshold <- (results_G_complete$adjP<=0.15 & abs(results_G_complete$DB)>=0.1)
...
max(results_G_complete[results_G_complete$threshold==TRUE,]$`Pr...t..`) #0.01229916
...
library(ggrepel)
png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/hydroxy/WTSE_TGSE/linear_model/CpG/volcano_WTSE_TGSE_CpG_thresholded.png", width=305, height=335)
ggplot(data=results_G_complete, aes(x=DB, y=-log10(adjP), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 adj P-Value") + theme_classic() + scale_color_manual(values=c("gray60","gray60", "gray87")) + geom_hline(yintercept=-log10(0.05)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none") + geom_label_repel(data = results_G_complete[results_G_complete$threshold==TRUE,], aes(label = gene), size = 4, force = 10, max.overlaps = 30, min.segment.length=0)
dev.off()

summary(results_G_complete$DB)
max(-log10(results_G_complete$Pr...t..))

png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/hydroxy/WTSE_TGSE/linear_model/CpG/volcano_WTSE_TGSE_CpG_thresholded_nomP.png", width=305, height=335)
ggplot(data=results_G_complete, aes(x=DB, y=-log10(`Pr...t..`), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 P-Value") + theme_classic() + scale_color_manual(values=c("gray60", "gray60","gray87")) + geom_hline(yintercept=-log10(0.01229916)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none")  + geom_label_repel(data = results_G_complete[results_G_complete$threshold==TRUE,], aes(label = gene), size = 4, force = 10, max.overlaps = 30, min.segment.length=0) + xlim(c(-0.75,0.75)) + ylim(c(0,7))
dev.off()
...
nrow(betas_sub <- betas_variable[rownames(betas_variable) %in% results_G_complete[results_G_complete$threshold==TRUE,"site"],]) #1583
