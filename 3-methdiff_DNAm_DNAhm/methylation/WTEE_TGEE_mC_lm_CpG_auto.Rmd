...
png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/volcano_GxE_CpG_auto.png", width=305, height=335)
...
write.csv(results_G, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/lm_WTEE_TGEE.csv", row.names=F)

png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/volcano_GxE_CpG_auto_manual_DB.png", width=305, height=335)
...
#LM on variable cytosines with % aligned adjustment
lm_results_G <- lapply(1:nrow(betas), function(x) {
  return(summary(lm(betas[x,] ~ meta$Genotype*meta$Environment + meta$Percent_aligned))$coefficients[2,])
})
results_G <- as.data.frame(do.call(rbind, lm_results_G))
results_G$z <- zscoreT(results_G$`t value`, df=12)
results_G$site <- rownames(betas)
...
png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/auto/CpG/volcano_01272023.png", width=305, height=335)
ggplot(data=results_G, aes(x=DB, y=-log10(`Pr(>|t|)`), colour="grey")) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 P-Value") + theme_classic() + scale_color_manual(values=c( "gray87")) + theme(legend.position = "none")
dev.off()
```
![P-value histogram](/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/pval_hist.png)

![Q-Q plot](/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/qqplot.png)

![Volcano plot](/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/volcano_GxE_CpG_auto.png)
...
#T = 0.050883, p-value < 2.2e-16

#get the nominal p threshold which corresponds to your FDR cutoff
max(results_G_complete[results_G_complete$threshold==TRUE,]$`Pr...t..`) #5.243391e-06
...
library(ggrepel)
png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/volcano_WTEE_TGEE_CpG_thresholded.png", width=305, height=335)
ggplot(data=results_G_complete, aes(x=Estimate, y=-log10(adjP), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 adj P-Value") + theme_classic() + scale_color_manual(values=c("palegreen3", "gray87")) + geom_hline(yintercept=-log10(0.05)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none") 
dev.off()
...
write.csv(results_G_complete, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/lm_WTEE_TGEE.csv", row.names=F)
...
ggplot(data=results_G_complete, aes(x=Estimate, y=-log10(`Pr...t..`), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 P-Value") + theme_classic() + scale_color_manual(values=c("palegreen3", "palegreen3","gray87")) + geom_hline(yintercept=-log10(5.243391e-06)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none")  + geom_label_repel(data = results_G_complete[results_G_complete$threshold==TRUE,], aes(label = gene), size = 4, force = 10, max.overlaps = 30, min.segment.length=0) + xlim(c(-0.75,0.75))
dev.off()

write.csv(results_G_complete, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/lm_WTEE_TGEE.csv", row.names=F)
hits <- results_G_complete[results_G_complete$threshold==TRUE,c(6,11,13,14,7,8)]
write.csv(hits, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpG/lm_WTEE_TGEE_mC_hits.csv", row.names=F)
```
![Volcano plot with thresholds](/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/volcano_WTEE_TGEE_CpG_thresholded_nomP.png)
...
betas_sub <- as.matrix(all_EE_sub[,1:13])
