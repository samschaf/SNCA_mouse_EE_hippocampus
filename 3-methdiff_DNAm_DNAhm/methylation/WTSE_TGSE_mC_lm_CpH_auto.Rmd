...
ggplot(data=results_G_complete, aes(x=Estimate, y=-log10(adjP), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Adjusted Delta Beta") + ylab("-log10 adj P-Value") + theme_classic() + scale_color_manual(values=c("gray60","gray60", "gray87")) + geom_hline(yintercept=-log10(0.05)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none") + ylim(c(0,15)) + xlim(c(-0.8,0.8))
dev.off()

png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTSE_TGSE/linear_model/CpH/volcano_WTSE_TGSE_CpH_thresholded_nomP.png", width=305, height=335)
ggplot(data=results_G, aes(x=Estimate, y=-log10(`Pr...t..`), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 P-Value") + theme_classic() + scale_color_manual(values=c("gray60","gray60", "gray87")) + geom_hline(yintercept=-log10(8.287476e-06)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none")  + ylim(c(0,15)) + xlim(c(-0.8,0.8))
dev.off()
...
write.csv(results_G_complete, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTSE_TGSE/linear_model/CpH/lm_WTSE_TGSE_CpH.csv", row.names=F)

hits <- results_G_complete[results_G_complete$threshold==TRUE,c(6,11,13,14,7,8)]
write.csv(hits, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTSE_TGSE/linear_model/CpH/lm_WTSE_TGSE_mC_CpH_hits.csv", row.names=F)
```
