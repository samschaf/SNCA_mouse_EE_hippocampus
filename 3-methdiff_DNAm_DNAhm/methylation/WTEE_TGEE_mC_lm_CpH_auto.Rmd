...
library(ggrepel)
png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpH/volcano_WTEE_TGEE_CpH_thresholded.png", width=305, height=335)
ggplot(data=results_G_complete, aes(x=Estimate, y=-log10(adjP), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 adj P-Value") + theme_classic() + scale_color_manual(values=c("palegreen3", "palegreen3","gray87")) + geom_hline(yintercept=-log10(0.05)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none") 
dev.off()

png("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpH/volcano_WTEE_TGEE_CpH_thresholded_nomP.png", width=305, height=335)
ggplot(data=results_G, aes(x=Estimate, y=-log10(`Pr...t..`), colour=DNAm_change)) +
  geom_point(size=1.75) +
  labs(legend.position = "none") +
  xlab("Delta Beta") + ylab("-log10 P-Value") + theme_classic() + scale_color_manual(values=c("palegreen3","palegreen3", "gray87")) + geom_hline(yintercept=-log10(1.718979e-05)) + geom_vline(xintercept=-0.1) + geom_vline(xintercept=0.1) + theme(legend.position = "none") + xlim(c(-0.8,0.8)) + ylim(c(0,15))
dev.off()

#annotate genes
anno <- read.delim("/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/preprocessing/oxbisulfite/CpH/CpH_anno_mC_ChIP.txt")
anno$site <- gsub(": ", ".", anno$site)
all(anno$site==results_G_complete$site) #TRUE
results_G_complete <- cbind(results_G_complete, anno[,2:5])
write.csv(results_G_complete, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpH/lm_WTEE_TGEE_CpH.csv", row.names=F)

hits <- results_G_complete[results_G_complete$threshold==TRUE,c(6,11,13,14,7,8)]
write.csv(hits, file="/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpH/lm_WTEE_TGEE_mC_CpH_hits.csv", row.names=F)
```
![Volcano plot with thresholds](/mnt/scratch/KoborLab/DecipherPD_mouse/ee_asyn/hipp/sschaffner/082022/5-methdiff_DNAm_DNAhm/methylation/WTEE_TGEE/linear_model/CpH/volcano_WTEE_TGEE_CpH_thresholded_nomP.png)
