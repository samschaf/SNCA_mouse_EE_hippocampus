...
H3K27ac_WTSE_TGSE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/H3K27ac/H3K27ac_WTSE-TGSE_anno.csv")
...
H3K27ac_WTEE_TGEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/H3K27ac/H3K27ac_WTEE-TGEE_anno.csv")
#removing Gulp1 (SNCA integration site)
...
#Plotting genomic context enrichment for all SE peaks
...
length(WTSE_TGSE_hits <- WTSE_TGSE_hits[complete.cases(WTSE_TGSE_hits)]) #1024 DB genes
WTSE_TGSE_up <- unique(H3K27ac_WTSE_TGSE[H3K27ac_WTSE_TGSE$FDR<=0.05 & H3K27ac_WTSE_TGSE$Fold>=0.5,"SYMBOL"])
length(WTSE_TGSE_up <- WTSE_TGSE_up[complete.cases(WTSE_TGSE_up)]) #703 DB genes
WTSE_TGSE_down <- unique(H3K27ac_WTSE_TGSE[H3K27ac_WTSE_TGSE$FDR<=0.05 & H3K27ac_WTSE_TGSE$Fold<=-0.5,"SYMBOL"])
length(WTSE_TGSE_down <- WTSE_TGSE_down[complete.cases(WTSE_TGSE_down)]) #366 DB genes
...
length(background_genes <- unique(H3K27ac_WTSE_TGSE$SYMBOL[complete.cases(H3K27ac_WTSE_TGSE$SYMBOL)])) #16,858

# reduce Linnarsson set to common genes
nrow(linnarsson <- linnarsson[linnarsson$V1 %in% background_genes,]) #14,338
...
#int
length(int_genes_all <- intersectsAll$intersect1) #66 genes
length(classes$class1) #1126 total
#5.9% of total

#pyrN
length(pyrN_genes_all <- intersectsAll$intersect3) #152 genes
length(classes$class3) #1599 total
#9.5% of total

#oligo
length(oligo_genes_all <- intersectsAll$intersect4) #52 genes
length(classes$class4) #579 total
#9% of total

#microglia
length(micro_genes_all <- intersectsAll$intersect5) #11 genes
length(classes$class5) #392 total
#2.8% of total

#astrocytes
length(astro_genes_all <- intersectsAll$intersect7) #12 genes
length(classes$class7) #206 total
#5.8% of total
...
heatmap.2(as.matrix(foldChanges),
          col = hmcols,
          #breaks = colbreaks,
...
```{r celltype, eval=F}
#get list of differentially bound genes
WTEE_TGEE_hits <- unique(H3K27ac_WTEE_TGEE[H3K27ac_WTEE_TGEE$FDR<=0.05 & abs(H3K27ac_WTEE_TGEE$Fold)>=0.5,"SYMBOL"])
length(WTEE_TGEE_hits <- WTEE_TGEE_hits[complete.cases(WTEE_TGEE_hits)]) #1062 DB genes
WTEE_TGEE_up <- unique(H3K27ac_WTEE_TGEE[H3K27ac_WTEE_TGEE$FDR<=0.05 & H3K27ac_WTEE_TGEE$Fold>=0.5,"SYMBOL"])
length(WTEE_TGEE_up <- WTEE_TGEE_up[complete.cases(WTEE_TGEE_up)]) #663 DB genes
WTEE_TGEE_down <- unique(H3K27ac_WTEE_TGEE[H3K27ac_WTEE_TGEE$FDR<=0.05 & H3K27ac_WTEE_TGEE$Fold<=-0.5,"SYMBOL"])
length(WTEE_TGEE_down <- WTEE_TGEE_down[complete.cases(WTEE_TGEE_down)]) #448 DB genes
...
# caclulate common genes
#idx = na.omit(match(myMap$mgi_symbol , linnarsson$V1))
length(background_genes <- unique(H3K27ac_WTEE_TGEE$SYMBOL[complete.cases(H3K27ac_WTEE_TGEE$SYMBOL)])) #16,858
