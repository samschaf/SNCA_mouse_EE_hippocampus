...
H3K4me3_WTSE_TGSE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/H3K4me3/H3K4me3_WTSE-TGSE_anno.csv")
...
H3K4me3_WTEE_TGEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/H3K4me3/H3K4me3_WTEE-TGEE_anno.csv")
#removing Gulp1 (SNCA integration site)
...
WTEE_TGEE_hits <- unique(H3K4me3_WTEE_TGEE[H3K4me3_WTEE_TGEE$FDR<=0.05 & abs(H3K4me3_WTEE_TGEE$Fold)>=0.5,"SYMBOL"])
length(WTEE_TGEE_hits <- WTEE_TGEE_hits[complete.cases(WTEE_TGEE_hits)]) #196 DB genes
WTEE_TGEE_up <- unique(H3K4me3_WTEE_TGEE[H3K4me3_WTEE_TGEE$FDR<=0.05 & H3K4me3_WTEE_TGEE$Fold>=0.5,"SYMBOL"])
length(WTEE_TGEE_up <- WTEE_TGEE_up[complete.cases(WTEE_TGEE_up)]) #39 DB genes
WTEE_TGEE_down <- unique(H3K4me3_WTEE_TGEE[H3K4me3_WTEE_TGEE$FDR<=0.05 & H3K4me3_WTEE_TGEE$Fold<=-0.5,"SYMBOL"])
length(WTEE_TGEE_down <- WTEE_TGEE_down[complete.cases(WTEE_TGEE_down)]) #159 DB genes
...
length(background_genes <- unique(H3K4me3_WTEE_TGEE$SYMBOL[complete.cases(H3K4me3_WTEE_TGEE$SYMBOL)])) #16,779

# reduce Linnarsson set to common genes
nrow(linnarsson <- linnarsson[linnarsson$V1 %in% background_genes,]) #14,037
...
for (i in 1:9){
  fisherMatrix = matrix( c(lengths(intersectsAll[i]) , #DBGs in each cell type class
                           ...
                           length(WTEE_TGEE_hits) - lengths(intersectsAll[i]), #DBGs not in class
                           dim(linnarsson)[1] - length(WTEE_TGEE_hits) - (lengths(classes[i]) - lengths(intersectsAll[i])) #notDBGs not in class
                         ),
                         nrow = 2,
