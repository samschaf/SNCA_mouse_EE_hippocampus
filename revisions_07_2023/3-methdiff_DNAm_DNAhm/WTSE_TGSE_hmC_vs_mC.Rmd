...
hmC_up <- results_G[results_G$DB>0 & results_G$threshold==TRUE & results_G$adjP<=0.05,"site"] #10 CpGs

#CpGs with decreased hmC in TG mice
hmC_down <- results_G[results_G$DB<0 & results_G$threshold==TRUE & results_G$adjP<=0.05,"site"] #697 CpGs
...
#DNAm
nrow(betas_mC <- all_EE[rownames(all_EE) %in% hmC_up,]) #10
...
nrow(betas_hmC <- betas_variable[rownames(betas_variable) %in% hmC_up,]) #10
...
####### CpGs with decreased hmC in TG mice 
#DNAm
nrow(betas_mC <- all_EE[rownames(all_EE) %in% hmC_down,]) #697

#DNAhm
nrow(betas_hmC <- betas_variable[rownames(betas_variable) %in% hmC_down,]) #697
all(rownames(betas_mC)==rownames(betas_hmC)) #TRUE
