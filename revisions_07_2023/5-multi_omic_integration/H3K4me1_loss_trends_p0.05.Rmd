## Investigating genes with differential H3K4me1/mRNA expression
### Samantha Schaffner
### June 20, 2023
...
# Retrieving genes with differential H3K4me1 and mRNA expression in SE
```{r gene list, eval=F}
...
#exclude the 2 genes that also had differential H3K27ac
...
length(set21 <- H3K4me1[H3K4me1 %in% mRNA]) #36
length(set21 <- set21[-which(set21 %in% H3K27ac)]) #34
```
...
## mRNA expression
```{r}
RNA_WTSE_TGSE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/4-processed_RNAseq_data/mmus_hippo12m_TG_STD_vs_WT_STD.csv")
RNA_WTSE_WTEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/4-processed_RNAseq_data/mmus_hippo12m_WT_EE_vs_WT_STD.csv")
RNA_WTEE_TGEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/4-processed_RNAseq_data/mmus_hippo12m_TG_EE_vs_WT_EE.csv")
RNA_TGSE_TGEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/4-processed_RNAseq_data/mmus_hippo12m_TG_EE_vs_TG_STD.csv")

all.equal(RNA_WTSE_TGSE$ensembl, RNA_WTSE_WTEE$ensembl, RNA_WTEE_TGEE$ensembl, RNA_TGSE_TGEE$ensembl) #TRUE
all.equal(RNA_WTSE_TGSE$mgi_symbol, RNA_WTSE_WTEE$mgi_symbol, RNA_WTEE_TGEE$mgi_symbol, RNA_TGSE_TGEE$mgi_symbol)
#[1] "'is.NA' value mismatch: 1403 in current 3219 in target"
#not all ensembl genes annotated to mgi symbol in all data frames?

all.equal(RNA_WTSE_WTEE$mgi_symbol, RNA_WTEE_TGEE$mgi_symbol, RNA_TGSE_TGEE$mgi_symbol) #TRUE (WTSE TGSE was off)

#double check annotation of differentially expressed genes
RNA_WTSE_TGSE$mgi_symbol2 <- RNA_WTEE_TGEE$mgi_symbol
WTSE_TGSE_RNA2 <- RNA_WTSE_TGSE[abs(RNA_WTSE_TGSE$log2FoldChange)>=0.3 & RNA_WTSE_TGSE$padj<=0.05,]
length(mRNA <- unique(as.character(WTSE_TGSE_RNA$mgi_symbol[complete.cases(WTSE_TGSE_RNA$mgi_symbol)]))) #166 genes - these were all present in the original data frame.

n <- nrow(RNA_WTSE_TGSE)
RNA_FC <- data.frame(Ensembl=RNA_WTSE_TGSE$ensembl, Gene=RNA_WTEE_TGEE$mgi_symbol,Comparison=c(rep("TGSE_WTSE",n), rep("WTEE_WTSE",n), rep("TGEE_WTEE",n), rep("TGEE_TGSE",n)), log2FC=c(RNA_WTSE_TGSE$log2FoldChange, RNA_WTSE_WTEE$log2FoldChange, RNA_WTEE_TGEE$log2FoldChange, RNA_TGSE_TGEE$log2FoldChange), padj=c(RNA_WTSE_TGSE$padj, RNA_WTSE_WTEE$padj, RNA_WTEE_TGEE$padj, RNA_TGSE_TGEE$padj))
...
length(unique(RNA_FC_sub$Ensembl)) #34
length(unique(RNA_FC_sub$Gene)) #34 with mgi symbol
...
mRNA_TGEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/4-processed_RNAseq_data/mmus_hippo12m_TG_EE_vs_TG_STD.csv")
...
counts_sub <- nCounts_mnc50[rownames(nCounts_mnc50) %in% ensgenes,] #34 genes
counts_sub <- as.data.frame(t(counts_sub))
all.equal(rownames(counts_sub), sampleInfo$run) #TRUE
...
```{r}
H3K4me1_conc <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/conc_H3K4me1_2.csv")
H3K4me1_TGEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/H3K4me1/H3K4me1_TGSE-TGEE_anno.csv")
...
H3K4me1_WTSE_TGSE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/H3K4me1/H3K4me1_WTSE-TGSE_anno.csv")
...
H3K4me1_WTEE_TGEE <- read.csv("~/KoborLab/Projects/DecipherPD_mouse/EE_asyn/sschaffner/5-ChIPseq/diffbind_2_reps_all_conspeaks/H3K4me1/H3K4me1_WTEE-TGEE_anno.csv")
#removing Gulp1 (SNCA integration site)
...
length(unique(H3K4me1_scaled$Gene)) #34 genes
length(unique(H3K4me1_scaled$coord)) #45 peaks
summary(H3K4me1_scaled$feature)
#  Poised Enhancer   Active Enhancer   Active Promoter          Promoter            5' UTR              Exon            Intron 
#                2                 7                 6                 4                 0                 2                16 
...
nrow(K4me1_means <- K4me1_melt[,c("coord","Gene","feature","group","gene_mean","mean_TGSE","lab")]) #720
library(dplyr)
nrow(K4me1_means <- distinct(K4me1_means)) #180
